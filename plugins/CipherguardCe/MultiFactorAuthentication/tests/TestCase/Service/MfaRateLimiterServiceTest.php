...
/**
 * @covers \Cipherguard\MultiFactorAuthentication\Service\MfaRateLimiterService
 */
class MfaRateLimiterServiceTest extends AppTestCase
{
    /**
     * @var \Cipherguard\MultiFactorAuthentication\Service\MfaRateLimiterService
     */
    private $service;
    ...
    public function testMfaRateLimiterService_SessionAuthWithSpecifiedValue_FailedAttemptsExceeded()
    {
        ...
        Configure::write('cipherguard.security.mfa.maxAttempts', 1);
        // login action
        ActionLogFactory::make(['created' => FrozenTime::now()->subMinutes(2)])
            ->userId($user->id)
            ->loginAction()
    ...
    public function testMfaRateLimiterService_SessionAuthWithZeroInfiniteMaxAttempts()
    {
        ...
        Configure::write('cipherguard.security.mfa.maxAttempts', 0);
        // login action
        ActionLogFactory::make(['created' => FrozenTime::now()->subMinutes(2)])
            ->userId($user->id)
            ->loginAction()
    ...
    public function testMfaRateLimiterService_WithSpecifiedValue_FailedAttemptsNotExceeded(
        ...
        Configure::write('cipherguard.security.mfa.maxAttempts', 2);
        // login action
