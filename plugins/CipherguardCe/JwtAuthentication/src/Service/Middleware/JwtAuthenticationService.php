 ...
 * @since         3.3.0
 */
namespace Cipherguard\JwtAuthentication\Service\Middleware;
...
class JwtAuthenticationService extends AuthenticationService
{
    ...
    public function authenticate(ServerRequestInterface $request): ResultInterface
    {
        ...
        if ($this->isLoginEndpointPost($request)) {
            $this->loadAuthenticator('Cipherguard/JwtAuthentication.GpgJwt');
        } elseif ($this->isRefreshEndpointPost($request)) {
            $this->loadAuthenticator('Cipherguard/JwtAuthentication.JwtRefreshToken');
        } else {
