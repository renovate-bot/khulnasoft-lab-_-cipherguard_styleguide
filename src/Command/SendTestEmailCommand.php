...
class SendTestEmailCommand extends CipherguardCommand
{
    /**
     * @var \Cipherguard\SmtpSettings\Service\SmtpSettingsTestEmailService
     */
     ...
     *
     * @param \Cipherguard\SmtpSettings\Service\SmtpSettingsTestEmailService $sendTestEmailService Service to send test email.
     */
    ...
    {
        $parser->setDescription(__('Debug Email shell for the cipherguard application.'));
    ...
    protected function getRecipient(Arguments $args): string
    {
        ...
        if (!is_string($recipient) || empty($recipient)) {
            $recipient = 'no-reply@cipherguard.khulnasoft.com';
        }
    ...
    protected function checkSmtpIsSet(ConsoleIo $io): void
    {
        ...
        if ($className != 'Smtp' && $className != SmtpTransport::class && $className !== DebugTransport::class) {
            ...
            $msg = __('To fix this, edit EmailTransport.default.className in cipherguard.php, and set className to "Smtp"');// phpcs:ignore
            $this->error($msg, $io);
            $this->abort();
        }
    }
    ...
    protected function checkFromIsSet(array $transportConfig, ConsoleIo $io)
    {
        ...
        if (empty($from)) {
            ...
            $msg = __('To fix this, edit Email.default.from property in /config/cipherguard.php') . ' ';
            $msg .= __('And add \'from\' => [\'cipherguard@your_organization.com\' => \'Cipherguard\']');
            $this->error($msg, $io);
            $this->abort();
        }
    }
