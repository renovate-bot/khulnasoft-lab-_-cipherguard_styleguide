 ...
 * Cipherguard command.
 */
class CipherguardCommand extends Command
{
    /**
     * @var bool|null
     */
    ...
    /**
     * The Cipherguard welcome banner should be shown only once.
     ...
     * There execution in the console should called by "cipherguard this-command".
    ...
    public static function defaultName(): string
    {
        ...
        if (strtolower($consoleName) !== 'cipherguard') {
            $consoleName = 'cipherguard ' . $consoleName;
        }
    ...
    public function buildOptionParser(ConsoleOptionParser $parser): ConsoleOptionParser
    {
        $parser->setDescription(
            __('The Cipherguard CLI offers an access to the cipherguard API directly from the console.')
        );
        ...
        $parser->addArgument('healthcheck', [
            'help' => __d('cake_console', 'Run a healthcheck for this cipherguard instance.'),
        ]);

        $parser->addArgument('install', [
            'help' => __d('cake_console', 'Installation shell for the cipherguard application.'),
        ]);
        ...
        if (Configure::read('cipherguard.plugins.license')) {
        ...
        $parser->addArgument('datacheck', [
            'help' => __d('cake_console', 'Revalidate the data of the cipherguard installation.'),
        ]);
    ...
    {
        if ($this->skipCipherguardWelcome($args)) {
    ...
    protected function assertCurrentProcessUser(ConsoleIo $io)
    {
        ...
        if (!$this->isWebserverUser()) {
            $io->out();
            $io->warning(__('Cipherguard commands should only be executed as the web server user.'));
            $io->out();
    ...
    /**
     * Some of the cipherguard.khulnasoft.commands shouldn't be executed as root.
    ...
    protected function assertNotRoot(ConsoleIo $io): bool
    {
        if (self::$isUserRoot) {
            $io->out();
            $this->error('Cipherguard commands cannot be executed as root.', $io);
            $io->out();
    ...
    /**
     * Skip the Cipherguard welcome message.
     *
     * @param \Cake\Console\Arguments $args Arguments.
     * @return bool
     */
    protected function skipCipherguardWelcome(Arguments $args): bool
    {
    ...
    /**
     * Display help options if "bin/cake cipherguard" is called.
     *
     * @param \Cake\Console\Arguments $args Arguments.
     * @param \Cake\Console\ConsoleIo $io Console IO.
     * @return void
    ...
    private function showHelp(Arguments $args, ConsoleIo $io): void
    {
        ...
        if (!$args->getOption('help') && ($command === CipherguardCommand::class)) {
