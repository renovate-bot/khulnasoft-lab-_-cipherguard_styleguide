...
class GpgHealthchecks
{
    ...
    {
        $checks['gpg']['gpgKey'] = (Configure::read('cipherguard.gpg.serverKey.fingerprint') !== null);
        $default = '2FC8945833C51946E937F9FED47B0811573EE67E';
        $checks['gpg']['gpgKeyNotDefault'] = (Configure::read('cipherguard.gpg.serverKey.fingerprint') !== $default);
    ...
    {
        switch (Configure::read('cipherguard.gpg.backend')) {
    ...
    {
        $checks['gpg']['gpgKeyPublic'] = (Configure::read('cipherguard.gpg.serverKey.public') !== null);
        $checks['gpg']['gpgKeyPublicReadable'] = is_readable(Configure::read('cipherguard.gpg.serverKey.public'));
        if ($checks['gpg']['gpgKeyPublicReadable']) {
            $publicKeydata = file_get_contents(Configure::read('cipherguard.gpg.serverKey.public'));
            $blockStart = '-----BEGIN PGP PUBLIC KEY BLOCK-----';
            $checks['gpg']['gpgKeyPublicBlock'] = (strpos($publicKeydata, $blockStart) === 0);
        }
        $checks['gpg']['gpgKeyPrivate'] = (Configure::read('cipherguard.gpg.serverKey.private') !== null);
        $checks['gpg']['info']['gpgKeyPrivate'] = Configure::read('cipherguard.gpg.serverKey.private');
        $checks['gpg']['gpgKeyPrivateReadable'] = is_readable(Configure::read('cipherguard.gpg.serverKey.private'));
        if ($checks['gpg']['gpgKeyPrivateReadable']) {
            $privateKeydata = file_get_contents(Configure::read('cipherguard.gpg.serverKey.private'));
    ...
    public static function gpgFingerprint(?array $checks = []): array
    {
        ...
        if ($areKeysReadable && $checks['gpg']['gpgKey']) {
            ...
            if ($privateKeyInfo['fingerprint'] === Configure::read('cipherguard.gpg.serverKey.fingerprint')) {
                $checks['gpg']['gpgKeyPrivateFingerprint'] = true;
            }
            $publicKeydata = file_get_contents(Configure::read('cipherguard.gpg.serverKey.public'));
            $publicKeyInfo = $gpg->getPublicKeyInfo($publicKeydata);
            if ($publicKeyInfo['fingerprint'] === Configure::read('cipherguard.gpg.serverKey.fingerprint')) {
    ...
    public static function gpgCanEncrypt(?array $checks = []): array
    {
        ...
        if ($checks['gpg']['gpgKeyPublicInKeyring']) {
            ...
            try {
                $_gpg->setEncryptKeyFromFingerprint(Configure::read('cipherguard.gpg.serverKey.fingerprint'));
    ...
    public static function gpgCanEncryptSign(?array $checks = []): array
    {
        ...
        if ($checks['gpg']['gpgKeyPublicInKeyring']) {
            ...
            try {
                $fingerprint = Configure::read('cipherguard.gpg.serverKey.fingerprint');
                $passphrase = Configure::read('cipherguard.gpg.serverKey.passphrase');
    ...
    public static function gpgIsFormatGopengpgValid(?array $checks = []): array
    {
        ...
        $publicKey->armored_key = file_get_contents(Configure::read('cipherguard.gpg.serverKey.public'));
        $privateKey = new Gpgkey();
        $privateKey->armored_key = file_get_contents(Configure::read('cipherguard.gpg.serverKey.private'));
