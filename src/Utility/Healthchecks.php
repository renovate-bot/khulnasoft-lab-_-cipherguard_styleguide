...
class Healthchecks
{
    ...
    public static function application(?array $checks = []): array
    {
        ...
        $checks['application']['sslForce'] = Configure::read('cipherguard.ssl.force');
        $https = strpos(Configure::read('App.fullBaseUrl'), 'https') === 0;
        $checks['application']['sslFullBaseUrl'] = ($https !== false);
        $checks['application']['seleniumDisabled'] = !Configure::read('cipherguard.selenium.active');
        ...
        $checks['application']['jsProd'] = (Configure::read('cipherguard.js.build') === 'production');
        $sendEmailJson = json_encode(Configure::read('cipherguard.email.send'));
    ...
    public static function environment(?array $checks = []): array
    {
        ...
        $checks['environment']['pcre'] = Validation::alphaNumeric('cipherguard');
     ...
     *
     * @param \Cipherguard\JwtAuthentication\Service\AccessToken\JwtKeyPairService|null $jwtKeyPairService JWT Service
    ...
    public static function jwt(?JwtKeyPairService $jwtKeyPairService = null, array $checks = []): array
    {
        ...
        $checks['jwt']['isEnabled'] = (Configure::read('cipherguard.plugins.jwtAuthentication.enabled') === true);
    ...
    {
        // Since the plugin might be removed from various cipherguard solutions, we check
