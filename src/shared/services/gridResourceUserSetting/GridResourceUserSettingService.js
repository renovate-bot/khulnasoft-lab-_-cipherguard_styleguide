...
class GridResourceUserSettingService {
  ...
  async getSetting() {
    try {
      ...
      const gridSetting = await this.port.request("cipherguard.resources.get-grid-setting");
      if (gridSetting !== null) {
  ...
  async setSetting(gridSettingEntity) {
    ...
    await this.port.request("cipherguard.resources.set-grid-setting", gridSettingEntity.toJSON());
  }
}
