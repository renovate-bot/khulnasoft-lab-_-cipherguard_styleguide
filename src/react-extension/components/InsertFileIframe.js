...
class InsertFileIframe extends Component {
   ...
   */
  async loadIframe() {
    const portId = await this.props.port.request("cipherguard.port.generate-id", "FileIframe");
    this.iframeRef.current.contentWindow.location = `${this.props.browserExtensionUrl}webAccessibleResources/cipherguard-iframe-file.html?cipherguard=${portId}`;
  }
  ...
  render() {
    return (
      <iframe id="cipherguard-iframe-file" ref={this.iframeRef} className="cachette"/>
    );
  }
}
