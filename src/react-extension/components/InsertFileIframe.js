...
class InsertFileIframe extends Component {
   ...
   */
  async loadIframe() {
    const portId = await this.props.port.request("passbolt.port.generate-id", "FileIframe");
    this.iframeRef.current.contentWindow.location = `${this.props.browserExtensionUrl}webAccessibleResources/passbolt-iframe-file.html?passbolt=${portId}`;
  }
