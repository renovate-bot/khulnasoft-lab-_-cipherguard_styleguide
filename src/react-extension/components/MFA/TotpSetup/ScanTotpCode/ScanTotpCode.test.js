...
describe("ScanTotpCode", () => {
  describe("As a logged user I should be able to scan a qrcode to setup the totp provider", () => {
    ...
    it('I should be able to generate a qr code', async() => {
      ...
      expect(TotpCodeGeneratorService.generateUri).toHaveBeenCalledWith(expect.objectContaining({"issuer": "localhost", "label": "ada@cipherguard.khulnasoft.com"}));
      expect(QRCode.toDataURL).toHaveBeenCalledWith(expect.arrayContaining(
        [expect.objectContaining({
          data: expect.stringContaining("otpauth://totp/localhost:ada%40cipherguard.khulnasoft.com?issuer=localhost&"),
          "mode": "byte"
        }
        )],
    ...
    it('I should see an help box in the scan totp screen ', async() => {
      ...
      expect(page.helpBoxButton.getAttribute('href')).toEqual('https://help.cipherguard.khulnasoft.com/start');
    });
  });
});
