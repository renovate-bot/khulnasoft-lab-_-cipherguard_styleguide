...
class InsertRecoverAuthenticationIframe extends Component {
   ...
   */
  async loadIframe() {
    const portId = await this.props.port.request("cipherguard.port.generate-id", "Recover");
    this.iframeRef.current.contentWindow.location = `${this.props.browserExtensionUrl}webAccessibleResources/cipherguard-iframe-recover.html?cipherguard=${portId}`;
  }
  ...
  render() {
    return (
      <iframe id="cipherguard-iframe-recover" ref={this.iframeRef} className="full-screen"/>
    );
  }
}
