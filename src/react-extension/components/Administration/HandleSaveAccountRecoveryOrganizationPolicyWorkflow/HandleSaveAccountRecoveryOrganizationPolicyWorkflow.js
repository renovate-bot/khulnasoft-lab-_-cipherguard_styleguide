...
class HandleSaveAccountRecoveryOrganizationPolicyWorkflow extends React.Component {
  ...
  handleError(error) {
    ...
    if (error.name === "CipherguardApiFetchError" && error?.data?.body?.account_recovery_organization_public_key?.fingerprint?.isNotAccountRecoveryOrganizationPublicKeyFingerprintRule) {
