...
describe("DisplaySubscriptionKeyPage", () => {
  ...
  describe(' As AD I can see the subscription', () => {
    ...
    it('As AD I should see all details about the subscription', async() => {
      ...
      expect(page.helpContactSales.getAttribute("href")).toBe("https://www.cipherguard.khulnasoft.com/contact");
    });
    ...
    it('As AD I should be able to identify if the limit of users is exceeded', async() => {
      jest.spyOn(props.context, 'onGetSubscriptionKeyRequested').mockImplementationOnce(() => {
        throw new CipherguardSubscriptionError("users exceeded", mockSubscriptionUsersExceeded);
      });
      page = new DisplaySubscriptionKeyPage(props.context, props);
      await waitFor(() => {});

      expect(page.subscriptionDetailsTitle).toBe("Your subscription key is not valid.");
      expect(page.contactUs.getAttribute("href")).toBe(`https://www.cipherguard.khulnasoft.com/contact`);
      ...
      expect(props.navigationContext.onGoToNewTab).toHaveBeenCalledWith(`https://www.cipherguard.khulnasoft.com/subscription/ee/update/qty?subscription_id=${mockSubscriptionUsersExceeded.subscription_id}&customer_id=${mockSubscriptionUsersExceeded.customer_id}`);
    });

    it('As AD I should be able to identify if the key is expired', async() => {
      jest.spyOn(props.context, 'onGetSubscriptionKeyRequested').mockImplementationOnce(() => {
        throw new CipherguardSubscriptionError("key expired", mockSubscriptionExpired);
      });
      page = new DisplaySubscriptionKeyPage(props.context, props);
      await waitFor(() => {});
      ...
      await page.goToRenewKey();
      expect(props.navigationContext.onGoToNewTab).toHaveBeenCalledWith(`https://www.cipherguard.khulnasoft.com/subscription/ee/update/renew?subscription_id=${mockSubscriptionExpired.subscription_id}&customer_id=${mockSubscriptionExpired.customer_id}`);
    });
    ...
    it('As AD I should be able to identify if the key is missing', async() => {
      expect.assertions(4);
      jest.spyOn(props.context, 'onGetSubscriptionKeyRequested').mockImplementationOnce(() => {
        throw new CipherguardApiFetchError("missing key", {});
      });
      page = new DisplaySubscriptionKeyPage(props.context, props);
      await waitFor(() => {});
   ...
   */
    ...
    it('As AD I should see an error if no subscription key was found', async() => {
      expect.assertions(4);
      jest.spyOn(props.context, 'onGetSubscriptionKeyRequested').mockImplementationOnce(() => {
        throw new CipherguardApiFetchError("no subscription key", "");
      });
