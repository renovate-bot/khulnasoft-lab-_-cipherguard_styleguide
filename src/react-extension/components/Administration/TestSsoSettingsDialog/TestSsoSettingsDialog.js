...
class TestSsoSettingsDialog extends React.Component {
  ...
  async handleSignInTestClick(event) {
    ...
    try {
      ...
      const ssoToken = await this.props.context.port.request('cipherguard.sso.dry-run', this.props.configurationId);
      this.setState({
  ...
  async handleActivateSsoSettings(event) {
    ...
    try {
      this.setState({processing: true});
      await this.props.context.port.request("cipherguard.sso.activate-settings", this.props.configurationId, this.state.ssoToken);
      await this.props.context.port.request("cipherguard.sso.generate-sso-kit", this.props.provider.id);
