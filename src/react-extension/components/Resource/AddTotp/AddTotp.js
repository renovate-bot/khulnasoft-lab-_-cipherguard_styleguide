...
class AddTotp extends Component {
  ...
  render() {
    return (
      <DialogWrapper title={this.translate("Add TOTP")} className="add-totp-dialog"
        ...
        <form onSubmit={this.handleFormSubmit} noValidate>
          <div className="form-content">
            <div className={`input text required ${this.isFieldError("secret_key") ? "error" : ""} ${this.state.processing ? 'disabled' : ''}`}>
              <label htmlFor="add-totp-form-key">
                <Trans>Key</Trans> (<Trans>secret</Trans>){this.isFieldWarning("secret_key") && <Icon name="exclamation"/>}
              </label>
              <div className="input-wrapper-inline">
                <input id="add-totp-form-key" name="secret_key" maxLength="1024" type="text" onKeyUp={this.handleInputKeyUp}
...
export default  withTranslation('common')(AddTotp);
