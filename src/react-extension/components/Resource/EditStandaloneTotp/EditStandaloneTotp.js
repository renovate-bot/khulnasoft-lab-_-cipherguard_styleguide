...
class EditStandaloneTotp extends Component {
  ...
  async getDecryptedSecret() {
    return this.props.context.port.request("cipherguard.secret.decrypt", this.props.resource.id);
  }
        ...
        <form onSubmit={this.handleFormSubmit} noValidate>
          <div className="form-content">
            <div className={`input text required ${this.isFieldError("name") ? "error" : ""} ${this.hasAllInputDisabled ? 'disabled' : ''}`}>
              <label htmlFor="edit-standalone-totp-form-name"><Trans>Name</Trans> (<Trans>Label</Trans>){this.isFieldWarning("name") && <Icon name="exclamation" />}</label>
            ...
            <div className={`input text ${this.hasAllInputDisabled ? 'disabled' : ''}`}>
              <label htmlFor="edit-standalone-totp-form-uri"><Trans>URI</Trans> (<Trans>Issuer</Trans>){this.isFieldWarning("uri") && <Icon name="exclamation" />}</label>
