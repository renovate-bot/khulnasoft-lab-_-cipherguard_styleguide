...
class InsertLoginAuthenticationIframe extends Component {
   ...
   */
  async loadIframe() {
    const portId = await this.props.port.request("cipherguard.port.generate-id", "Auth");
    this.iframeRef.current.contentWindow.location = `${this.props.browserExtensionUrl}webAccessibleResources/cipherguard-iframe-login.html?cipherguard=${portId}`;
  }
  ...
  render() {
    return (
      <iframe id="cipherguard-iframe-login" ref={this.iframeRef} className="full-screen"/>
    );
  }
}
