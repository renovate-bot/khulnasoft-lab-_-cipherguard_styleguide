...
describe("AuthenticationRecoverContextProvider", () => {
  ...
  describe("AuthenticationRecoverContextProvider::initialize", () => {
    it("When the extension was just installed and the browser is Chrome, the machine state should be set to: INTRODUCE_EXTENSION", async() => {
      ...
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.first-install", jest.fn(() => Promise.resolve(true)));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);

      expect.assertions(3);
      await contextProvider.initialize();
      expect(props.context.port.requestListeners["cipherguard.recover.first-install"]).toHaveBeenCalled();
      expect(props.context.port.requestListeners["cipherguard.recover.start"]).toHaveBeenCalled();
    ...
    it("When the extension was already installed and the URL is for a lost-passphrase case and the user enrolled for the account-recovery program, the machine state should be set to: GENERATE_ACCOUNT_RECOVERY_GPG_KEY", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.has-user-enabled-account-recovery", jest.fn(() => Promise.resolve(true)));
      props.context.port.addRequestListener("cipherguard.recover.lost-passphrase-case", jest.fn(() => Promise.resolve(true)));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);

      expect.assertions(4);
      await contextProvider.initialize();
      expect(props.context.port.requestListeners["cipherguard.recover.lost-passphrase-case"]).toHaveBeenCalled();
      expect(props.context.port.requestListeners["cipherguard.recover.has-user-enabled-account-recovery"]).toHaveBeenCalled();
      expect(props.context.port.requestListeners["cipherguard.recover.start"]).toHaveBeenCalled();
    ...
    it("When the extension was already installed and the URL is for a lost-passphrase case and the user didn't enrolled for the account-recovery program, the machine state should be set to: HELP_CREDENTIALS_LOST", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.has-user-enabled-account-recovery", jest.fn(() => Promise.resolve(false)));
      props.context.port.addRequestListener("cipherguard.recover.lost-passphrase-case", jest.fn(() => Promise.resolve(true)));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
  ...
  describe("AuthenticationRecoverContextProvider::importGpgKey", () => {
    it("When the gpg key is imported with success, the machine state should be set to: VALIDATE_PASSPHRASE", async() => {
      ...
      expect(props.context.port.requestListeners["cipherguard.recover.import-key"]).toHaveBeenCalledWith("armored_key", undefined);
    ...
    it("When an invalid gpg key fails the import, the error should be rethrown and the machine state should remain on: IMPORT_GPG_KEY", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.import-key", jest.fn(() => Promise.reject(new GpgKeyError())));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
    ...
    it("When an error occurred during the gpg key import, the machine state should be set to: UNEXPECTED_ERROR", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.import-key", jest.fn(() => Promise.reject(new Error('Unexpected error'))));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
  ...
  describe("AuthenticationRecoverContextProvider::checkPassphrase", () => {
    it("When a passphrase check succeed the machine state should be set to: CHOOSE_SECURITY_TOKEN", async() => {
      ...
      expect(props.context.port.requestListeners["cipherguard.recover.verify-passphrase"]).toHaveBeenCalledWith("passphrase", undefined);
    ...
    it("When a wrong passphrase is requested to be checked, the error should be rethrown and the machine state should remain on: SIGN_IN", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.verify-passphrase", jest.fn(() => Promise.reject(new InvalidMasterPasswordError())));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
    ...
    it("If an unexpected error occurred the machine state should be set to: UNEXPECTED_ERROR", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.verify-passphrase", jest.fn(() => Promise.reject(new Error("Unexpected error"))));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
  ...
  describe("AuthenticationRecoverContextProvider::chooseSecurityToken", () => {
    it("When the security token preferences are set with success, the machine state should be set to: SIGNING_IN", async() => {
      ...
      expect(props.context.port.requestListeners["cipherguard.recover.set-security-token"]).toHaveBeenCalledWith({color: "black", textColor: "red"}, undefined);
      expect(props.context.port.requestListeners["cipherguard.recover.complete"]).toHaveBeenCalled();
      expect(props.context.port.requestListeners["cipherguard.recover.sign-in"]).toHaveBeenCalledWith(null, undefined);
    ...
    it("When the security token preferences cannot be set, the machine state should be set to: UNEXPECTED_ERROR", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.set-security-token", jest.fn(() => Promise.reject(new Error("Unexpected error"))));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
  ...
  describe("AuthenticationRecoverContextProvider::generateAccountRecoveryGpgKey", () => {
    it("When the account recovery gpg key is generated, the machine state should be set to: CHOOSE_ACCOUNT_RECOVERY_SECURITY_TOKEN", async() => {
      ...
      expect(props.context.port.requestListeners["cipherguard.recover.generate-account-recovery-request-key"]).toHaveBeenCalledWith({passphrase: "passphrase"}, undefined);
    ...
    it("When an unexpected error occurred during the account recovery gpg key generation, the machine state should be set to: UNEXPECTED_ERRRO", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.generate-account-recovery-request-key", jest.fn(() => Promise.reject(new Error("Unexpected error"))));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
  ...
  describe("AuthenticationRecoverContextProvider::chooseAccountRecoverySecurityToken", () => {
    it("When the security token preferences are set with success, the machine state should be set to: SIGNING_IN", async() => {
      ...
      props.context.port.addRequestListener("cipherguard.recover.initiate-account-recovery-request", onRequestAccountRecoveryMock);
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
  ...
  describe("AuthenticationRecoverContextProvider::requestHelpCredentialsLost", () => {
    it("When the user requests help to an administrator, the machine state should be set to: CHECK_MAILBOX", async() => {
      ...
      expect(props.context.port.requestListeners["cipherguard.recover.request-help-credentials-lost"]).toHaveBeenCalled();
    ...
    it("When the user fails to request help to an administrator, the machine state should be set to: UNEXPECTED_ERROR", async() => {
      const props = defaultProps();
      props.context.port.addRequestListener("cipherguard.recover.request-help-credentials-lost", jest.fn(() => Promise.reject(new Error("Unexpected error"))));
      const contextProvider = new AuthenticationRecoverContextProvider(props);
      mockComponentSetState(contextProvider);
