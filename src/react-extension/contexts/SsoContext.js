...
export class SsoContextProvider extends React.Component {
  ...
  async loadSsoConfiguration() {
    const ssoLocalConfiguredProvider = await this.props.context.port.request("cipherguard.sso.get-local-configured-provider");
  ...
  async runSignInProcess() {
    try {
      ...
      await this.props.context.port.request(`cipherguard.sso.sign-in-with-${provider}`);
      await this.props.context.port.request('cipherguard.auth.post-login-redirect');
    } catch (e) {
      console.error(e);
