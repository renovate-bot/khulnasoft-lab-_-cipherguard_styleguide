...
class ExtBootstrapSetup extends Component {
  ...
  handleRemoveIframeRequested() {
    this.props.port.on("cipherguard.setup-bootstrap.remove-iframe", this.removeSetupIframe.bind(this));
  }
   ...
   */
  removeSetupIframe() {
    const iframe = document.getElementById("cipherguard-iframe-setup");
    if (iframe) {
      iframe.remove();
    }
  }
